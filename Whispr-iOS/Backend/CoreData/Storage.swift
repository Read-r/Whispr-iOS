...
//  Whispr-iOS
//
//  Copied and modified from https://github.com/3lvis/DATAStack by Mikael Sundström on 2017-08-01.
...
class Storage: NSObject {
    private var storeType = StorageType.SQLite
    ...
    public lazy var mainContext: NSManagedObjectContext = {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(Storage.mainContextDidSave(_:)), name: .NSManagedObjectContextDidSave, object: context)
    ...
    private lazy var writerContext: NSManagedObjectContext = {
        let context = NSManagedObjectContext(concurrencyType: Storage.backgroundConcurrencyType())
        context.undoManager = nil
        context.mergePolicy = NSMergeByPropertyStoreTrumpMergePolicy
        context.persistentStoreCoordinator = self.persistentStoreCoordinator
    ...
    /**
     Initializes a Storage using the bundle name as the model name, so if your target is called ModernApp,
    ...
    /**
     Initializes a Storage using the provided model name.
    ...
    /**
     Initializes a Storage using the provided model name, bundle and storeType.
     - parameter modelName: The name of your Core Data model (xcdatamodeld).
     ...
     */
    public init(modelName: String, storeType: StorageType) {
        self.modelName = modelName
    ...
    /**
     ...
     based and doesn't save to disk, while the second one creates a .sqlite file and stores things there.
     */
    public init(modelName: String, bundle: Bundle, storeType: StorageType) {
        self.modelName = modelName
    ...
    /**
     Initializes a Storage using the provided model name, bundle, storeType and store name.
     - parameter modelName: The name of your Core Data model (xcdatamodeld).
     - parameter bundle: The bundle where your Core Data model is located, normally your Core Data model is in
     the main bundle but when using unit tests sometimes your Core Data model could be located where your tests
     are located.
     ...
     change that.
     */
    public init(modelName: String, bundle: Bundle, storeType: StorageType, storeName: String) {
        self.modelName = modelName
        self.modelBundle = bundle
        self.storeType = storeType
     ...
     */
    public init(modelName: String, bundle: Bundle, storeType: StorageType, storeName: String, containerURL: URL) {
        self.modelName = modelName
        self.modelBundle = bundle
        self.storeType = storeType
        self.storeName = storeName
    ...
    /**
     Initializes a Storage using the provided model name, bundle and storeType.
     - parameter model: The model that we'll use to set up your Storage.
     - parameter storeType: The store type to be used, you have .InMemory and .SQLite, the first one is memory
     based and doesn't save to disk, while the second one creates a .sqlite file and stores things there.
     */
    public init(model: NSManagedObjectModel, storeType: StorageType) {
        self.model = model
        self.storeType = storeType
    ...
    public func newDisposableMainContext() -> NSManagedObjectContext {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(Storage.newDisposableMainContextWillSave(_:)), name: NSNotification.Name.NSManagedObjectContextWillSave, object: context)
    ...
    public func newBackgroundContext() -> NSManagedObjectContext {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(Storage.backgroundContextDidSave(_:)), name: .NSManagedObjectContextDidSave, object: context)
    ...
    public func performInNewBackgroundContext(_ operation: @escaping (_ backgroundContext: NSManagedObjectContext) -> Void) {
        ...
        context.perform(Storage.performSelectorForBackgroundContext(), with: blockObject)
    }
...
//                }
        ...
        let mainContextBlock: @convention(block) () -> Void = {
            self.writerContext.perform(Storage.performSelectorForBackgroundContext(), with: writerContextBlockObject)
            DispatchQueue.main.async {
                completion?(writerContextError)
            }
        }
        let mainContextBlockObject: AnyObject = unsafeBitCast(mainContextBlock, to: AnyObject.self)
        self.mainContext.perform(Storage.performSelectorForBackgroundContext(), with: mainContextBlockObject)
    }
    ...
    func backgroundContextDidSave(_ notification: Notification) throws {
        if Thread.isMainThread/* && TestCheck.isTesting == false*/ {
            ...
            let blockObject: AnyObject = unsafeBitCast(contextBlock, to: AnyObject.self)
            self.mainContext.perform(Storage.performSelectorForBackgroundContext(), with: blockObject)
        }
    }
...
extension NSPersistentStoreCoordinator {
    func addPersistentStore(storeType: StorageType, bundle: Bundle, modelName: String, storeName: String?, containerURL: URL) throws {
...
extension NSError {
    convenience init(info: String, previousError: NSError?) {
        if let previousError = previousError {
            ...
            self.init(domain: "com.SyncDB.Storage", code: 9999, userInfo: userInfo)
        }
    }
}
